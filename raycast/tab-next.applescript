...
tell application "System Events"
    ...
    -- Calculate the next index (wrap around to 1 if at end)
    set nextIndex to currentIndex + 1
    if nextIndex > (count of appList) then
        set nextIndex to 1
    end if
    
    -- Activate the next application
    set nextApp to item nextIndex of appList
